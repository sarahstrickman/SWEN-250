...
int test()
{
	...
	printf( "\nTests for create_array_of_differences..........................\n" ) ;
	...
	p_products = create_array_of_differences( first_array, NULL, sizeof( first_array ) / sizeof( int ) ) ;
	assert( p_products == NULL,
		"Expected a NULL pointer since 2nd array is NULL" )
		? passcount++ : failcount++ ;
	// Test 9 -- expect NULL pointer
	p_products = create_array_of_differences( first_array, second_array, 0 ) ;
	assert( p_products == NULL,
		"Expected a NULL pointer since number of elements is 0" )
		? passcount++ : failcount++ ;
	// Test 10 -- expect valid pointer
	p_products = create_array_of_differences( first_array, second_array, sizeof( first_array ) / sizeof( int ) ) ;
	...
	assert( p_products && *p_products == 4,
	...
	assert( p_products && *( p_products + 5 ) == 4,
